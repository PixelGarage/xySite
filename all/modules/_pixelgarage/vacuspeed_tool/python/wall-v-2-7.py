...
# Splits a rectangle vertically using the horizontal edges of a contained gap (if any)
...
# Splits a rectangle horizontally using the vertical edges of a contained gap (if any)
...
# Splits marginal strip based on holes overlapping with it
...
# Determine the space between the edge of the wall and the closest grid point
def rIntDiv(x1, num):
    counter = 0
    ...
    for i in range(0, fillRows - maskRows + 1):
        for j in range(0, fillColumns - maskColumns + 1):
            ...
            if (score[9] < minAnomalies):
                points = int(score[8]) << 30 | int(score[4]) << 20 | int(score[2]) << 10 | int(score[1])
...
# Translate hole horizontally (mirror w.r. to the vertical axis of the wall) 
...
# Translate hole vertically (mirror w.r. to the horizontal axis of the wall)
...
# Trims holes to margins (width, height)
def trimHoles(width, height, holes):
    res = []
    for (a, b, c, d) in holes:
        # Ignore holes outside the (width, height) area
        if (a >= width):
            continue
        ...
        # Trim hole to margins (width, height)
        c2 = c
        if (c2 >= width):
            c2 = width
...
# All unique X coordinates of all tiles
def getAllX(tiles):
    result = []
...
# All unique Y coordinates of all tiles
def getAllY(tiles):
    result = []
...
def main():    
    ...
    parser.add_option('-l', '--layers', type = 'string', help = "Specifies number of layers. Values are: 1 or 2")
    ...
    # Determine the space between the rightmost edge of the wall and the rightmost grid points
    expandedRight = rIntDiv(wall[0], 25)
    
    # Determine the space between the lower edge of the wall and the bottom grid points
    expandedDown = rIntDiv(wall[1], 25)
    
    # Tiling data for each layer
    eR = [0, 0]
    eD = [0, 0]
    layer = [[], []]
    marginalTiles = [[], []]
    ...
    if (numLayers == 2):
        # In-Layer shifting strategies are defined here
        if (expandedRight <= 12.5):
            # Leave space on right hand side in layer 0
            eR[0] = expandedRight
            # Start on margin for layer 1
            eR[1] = 0
        else:
            # Split space between left and right margins
            firstRoundFlip = 1
            eR[0] = expandedRight
            #eR[1] = expandedRight - 12.5
    
        if (expandedDown <= 12.5):
            # Leave space on the lower part of the wall
            eD[0] = expandedDown
            eD[1] = 0
        else:
            # Split space between top and bottom margins
            firstRoundFlip = 1
            eD[0] = expandedDown
    ...
    if (expandedRight == 0):
        # Fill starting from left margin in first layer and break into halves in second layer
        eR[1] = 12.5

    if (expandedDown == 0):
        # Fill starting from top margin margin in first layer and break into halves in second layer
        eD[1] = 12.5

    # Old wall coordinates
    oldX = wall[0]
    oldY = wall[1]

    # Copy original holes array
    ...
    for i in range(0, 2):    
        ...
        rightTiles = []
        downTiles = []
        # Do we need to add marginal tiles?
        if (firstRoundFlip or (i != 0)):
            if (eR[i] > 0):
                # Actual width that will be filled with non-marginal tiles
                width = wall[0] - eR[i]
                # Determine part of holes or holes which fall within the area filled with regular tiles
                trimmedHoles = trimHoles(width, height, trimmedHoles)
                # Fill right hand side edge with marginal tiles taking account of overlapping holes
            ...
            # Perform horizontal mirror and vertical mirror (in this order). This is because we attempt to fill from lower-right corner
            (layer[i], marginalTiles[i], maxScore[i]) = runAlgorithm(width, height, THV(width, height, THH(width, height, trimmedHoles)), mode, 1 - i)

            # Perform another mirror operation to get the results represented in the original reference frame format
    ...
    if (dData):
        ...
        while 1:
            ...
            if len(Geesh) > 0:    
                if ((Geesh[0].type == pygame.KEYDOWN) or (Geesh[0].type == pygame.QUIT)) : break;
