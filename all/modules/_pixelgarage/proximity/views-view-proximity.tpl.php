...
<div id="<?php print $pe_container_id; ?>" class="pe-container">
  ...
  <?php foreach ($rows as $id => $row): ?>
    ...
    <div class="pe-item pe-item-ajax <?php print 'pe-item-' . $ajax_load_params[$id]; ?> <?php if ($classes_array[$id]) print $classes_array[$id]; ?>" style="margin-left: -4px">
      <div class="pe-item-inner">
        <!-- modal trigger -->
        <a class="btn" role="button" href="<?php print $item_base_url . $ajax_load_params[$id]; ?>" data-ajax-load-param="<?php print $ajax_load_params[$id]; ?>" <?php print drupal_attributes($toggle_attributes); ?>>
          <?php print $row; ?>
        </a>
      </div>
    </div>
